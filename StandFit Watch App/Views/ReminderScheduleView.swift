...
/// This addresses UX1: users now see what will change before it takes effect.
struct ReminderScheduleView: View {
    ...
    var body: some View {
        ...
        .onChange(of: pendingInterval) { _, _ in updateHasChanges() }
        .onChange(of: pendingActiveDays) { _, _ in updateHasChanges() }
        .onChange(of: pendingStartHour) { _, _ in updateHasChanges() }
        .onChange(of: pendingEndHour) { _, _ in updateHasChanges() }
        .onChange(of: pendingDeadResponseEnabled) { _, _ in updateHasChanges() }
        .onChange(of: pendingDeadResponseMinutes) { _, _ in updateHasChanges() }
    }
    ...
    private var activeHoursSection: some View {
        Section {
            ...
            .pickerStyle(.navigationLink)
            .onChange(of: pendingStartHour) { _, newValue in
            ...
            .pickerStyle(.navigationLink)
            .onChange(of: pendingEndHour) { _, newValue in
        ...
        Section {
            ...
            if pendingDeadResponseEnabled {
                Picker("Retry after", selection: $pendingDeadResponseMinutes) {
                    ForEach(ExerciseStore.deadResponseOptions, id: \.self) { minutes in
